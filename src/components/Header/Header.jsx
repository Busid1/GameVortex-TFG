...
export default function Header({ cartCount, onSearch, searchGame, videogames, handleRemoveFromCart }) {
    ...
    useEffect(() => {
        // Update cartGamesPrev when state.gamesInCart changes
        setCartGamesPrev(reduxGamesInCart);
    }, [reduxGamesInCart]); // This effect runs whenever reduxGamesInCart changes
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            // If the clicked element is not cartHeader or one of its descendants
            if (cartHeaderRef.current && !cartHeaderRef.current.contains(event.target)) {
                setIsPrevClose(false);
                // You can perform any actions you want when the cartHeader is not clicked here
            }
        };

        // Add event listener to the document to detect clicks outside the cartHeader
        document.addEventListener('mousedown', handleClickOutside);

        // It's important to clean up the event listener when the component unmounts
        return () => {
